...
@section('title', 'Manage Portfolio - Admin')
...
<div class="mb-8 flex justify-between items-center">
    <div>
        <h1 class="text-2xl font-bold text-slate-900">Manage Portfolio</h1>
        <p class="text-slate-500 mt-1">Upload and categorize previous work to showcase to customers.</p>
    </div>
</div>
    ...
    <div class="lg:col-span-1">
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-200 bg-slate-50">
                <h3 class="text-lg font-bold text-slate-900">Add Portfolio Item</h3>
            </div>
            <div class="p-6">
                <form action="{{ route('admin.portfolios.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="space-y-5">
                        <div>
                            ...
                            <input type="text" name="section" placeholder="e.g., Ecommerce, Shopify, UI/UX" required
                        ...
                        <button type="submit" 
                            ...
                            <i data-lucide="plus" class="w-4 h-4"></i> Add to Portfolio
                        </button>
                    </div>
                </form>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                            ...
                            @foreach($portfolios as $portfolio)
                                <tr x-data="{ editOpen: false }" class="hover:bg-slate-50 transition-colors group">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="h-20 w-28 rounded-lg overflow-hidden border border-slate-200 shadow-sm relative group cursor-pointer">
                                            <img src="{{ Storage::url($portfolio->image) }}" alt="{{ $portfolio->name }}" class="w-full h-full object-cover">
                                            <a href="{{ Storage::url($portfolio->image) }}" target="_blank" class="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <form action="{{ route('admin.portfolios.toggle', $portfolio) }}" method="POST">
                                            @csrf
                                            <button type="submit" class="inline-flex items-center gap-1.5 py-1 px-3 rounded-full text-xs font-semibold transition-colors {{ $portfolio->is_active ? 'bg-emerald-100 text-emerald-800 hover:bg-emerald-200' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                                                <i data-lucide="{{ $portfolio->is_active ? 'check-circle' : 'power-off' }}" class="w-3 h-3"></i>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        ...
                                        <form action="{{ route('admin.portfolios.destroy', $portfolio) }}" method="POST" class="inline" onsubmit="return confirm('Delete this portfolio item permanently?');">
                                            @csrf
                                            @method('DELETE')
                                        ...
                                        <div x-show="editOpen" 
                                            ...
                                            <div @click.away="editOpen = false" class="bg-white rounded-2xl shadow-xl border border-slate-200 w-full max-w-md overflow-hidden text-left">
                                                <div class="px-6 py-4 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
                                                    <h3 class="text-lg font-bold text-slate-900">Edit Portfolio Item</h3>
                                                    ...
                                                    <form action="{{ route('admin.portfolios.update', $portfolio) }}" method="POST" enctype="multipart/form-data">
                                                        ...
                                                        <div class="space-y-4">
                                                            <div>
                                                                ...
                                                                <input type="text" name="section" value="{{ $portfolio->section }}" required class="w-full border border-slate-200 bg-slate-50 text-slate-900 text-sm rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-indigo-500 outline-none">
                                                            </div>
                                                            <div>
                                                                <label class="block text-sm font-semibold text-slate-700 mb-1.5">Project Name</label>
                                                                <input type="text" name="name" value="{{ $portfolio->name }}" required class="w-full border border-slate-200 bg-slate-50 text-slate-900 text-sm rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-indigo-500 outline-none">
                                                            </div>
                                                            <div>
            ...
            @else
                <div class="p-12 text-center text-slate-500">
                    ...
                    <p class="font-medium text-slate-900">No portfolio items added yet.</p>
                    <p class="text-sm mt-1">Use the form on the left to add your first project.</p>
                </div>
            @endif
        </div>
    </div>
