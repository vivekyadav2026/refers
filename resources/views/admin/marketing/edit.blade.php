@extends('layouts.app')
@section('title', 'Edit Asset — Admin')
@section('sidebar')
    <!-- enable sidebar -->
@endsection
...
<div class="mb-8">
    <div class="flex items-center gap-2 mb-2">
        ...
        <h1 class="text-2xl font-black text-slate-900 tracking-tight">Edit Asset</h1>
    </div>
    <p class="text-slate-500 text-sm pl-7">Update details for this marketing material.</p>
</div>

<div class="max-w-3xl bg-white rounded-2xl shadow-sm border border-slate-200 p-6 sm:p-8">
    <form action="{{ route('admin.marketing.update', $marketing) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title', $marketing->title) }}" required class="w-full px-4 py-3 rounded-xl border border-slate-200 text-slate-900 focus:ring-2 focus:ring-indigo-500 bg-slate-50">
            ...
            <div>
                ...
                <select name="type" class="w-full px-4 py-3 rounded-xl border border-slate-200 text-slate-900 focus:ring-2 focus:ring-indigo-500 bg-slate-50">
                    <option value="image" {{ old('type', $marketing->type) == 'image' ? 'selected' : '' }}>Image (Banner, Poster)</option>
                    <option value="video" {{ old('type', $marketing->type) == 'video' ? 'selected' : '' }}>Video (Reel, Promo)</option>
                    <option value="document" {{ old('type', $marketing->type) == 'document' ? 'selected' : '' }}>Document (PDF Guide)</option>
                </select>
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full px-4 py-3 rounded-xl border border-slate-200 text-slate-900 focus:ring-2 focus:ring-indigo-500 bg-slate-50 resize-none">{{ old('description', $marketing->description) }}</textarea>
                @error('description') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-600 mb-2 uppercase tracking-wider">Upload New File <span class="text-slate-400">(Optional - Leave blank to keep current)</span></label>
                <div class="mt-1 flex justify-center px-6 pt-8 pb-10 border-2 border-slate-200 border-dashed rounded-xl hover:border-indigo-400 transition-colors bg-slate-50/50 cursor-pointer" onclick="document.getElementById('file').click()">
                    <div class="space-y-2 text-center">
                        ...
                        <p class="text-xs text-slate-500" id="file-name">Currently: {{ basename($marketing->file_path) }}</p>
                    </div>
                </div>
        ...
        <div class="mt-8 pt-6 border-t border-slate-100 flex justify-end gap-3">
            ...
            <button type="submit" class="px-6 py-3 rounded-xl text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 transition-colors shadow-sm">Update Asset</button>
        </div>
    </form>
</div>
