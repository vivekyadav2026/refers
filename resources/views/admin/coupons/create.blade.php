...
<div class="mb-8">
    <div class="flex items-center gap-2 mb-2">
        ...
        <h1 class="text-2xl font-black text-slate-900 tracking-tight">Create Coupon</h1>
    </div>
    <p class="text-slate-500 text-sm pl-7">Add a new discount code for your customers.</p>
</div>
    ...
    <form action="{{ route('admin.coupons.store') }}" method="POST">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="md:col-span-2">
                ...
                <input type="text" name="code" value="{{ old('code') }}" required class="w-full px-4 py-3 rounded-xl border border-slate-200 text-slate-900 focus:ring-2 focus:ring-indigo-500 bg-slate-50 uppercase tracking-widest font-mono" placeholder="SUMMER50">
            ...
            <div>
                ...
                <input type="number" step="0.01" name="discount_value" value="{{ old('discount_value') }}" required class="w-full px-4 py-3 rounded-xl border border-slate-200 text-slate-900 focus:ring-2 focus:ring-indigo-500 bg-slate-50" placeholder="e.g. 10">
            ...
            <div>
                ...
                <input type="number" step="0.01" name="min_order_amount" value="{{ old('min_order_amount') }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 text-slate-900 focus:ring-2 focus:ring-indigo-500 bg-slate-50" placeholder="e.g. 500">
            </div>
            ...
            <div>
                ...
                <input type="number" name="max_uses" value="{{ old('max_uses') }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 text-slate-900 focus:ring-2 focus:ring-indigo-500 bg-slate-50" placeholder="e.g. 100">
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <input type="datetime-local" name="expires_at" value="{{ old('expires_at') }}" class="w-full px-4 py-3 rounded-xl border border-slate-200 text-slate-900 focus:ring-2 focus:ring-indigo-500 bg-slate-50">
            </div>

            <div class="md:col-span-2 mt-2">
                <label class="flex items-center gap-3 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" class="w-5 h-5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" checked>
        ...
        <div class="mt-8 pt-6 border-t border-slate-100 flex justify-end gap-3">
            ...
            <button type="submit" class="px-6 py-3 rounded-xl text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-700 transition-colors shadow-sm">Save Coupon</button>
        </div>
    </form>
</div>
@endsection
